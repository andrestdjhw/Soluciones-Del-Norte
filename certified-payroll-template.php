...
$c = $is_en ? array(
	...
	'faq_l'    => 'FAQ',
	'faq_h2'   => 'What people ask about certified payroll.',
	'faq_note' => 'If yours isn’t here, ask it on the intake call.',
	'faq_alt'  => 'Soluciones del Norte team member working on a laptop',
	'faqs'     => array(
		array(
			'What is a wage determination, and why does it matter?',
			'It’s the rate schedule the awarding agency sets for the project — every classification on your crew has to match it. We verify it before the report goes out, not after.',
		),
		array(
			'What happens if a report is late or incomplete?',
			'The agency can hold your payment until it’s corrected. That’s exactly the scenario the certified cycle we run is built to avoid.',
		),
		array(
			'My project is private, not public — do I need this?',
			'No. Certified payroll only applies to state or city contracts; a private project needs standard payroll instead.',
		),
		array(
			'What do you need from me each cycle?',
			'The contract number and awarding agency, once, plus hours by worker and by classification every cycle.',
		),
	),
	...
	'faq_l'    => 'Preguntas',
	'faq_h2'   => 'Lo que se pregunta sobre nómina certificada.',
	...
	'faqs'     => array(
		array(
			'¿Qué es una determinación de salario y por qué importa?',
			'Es la tabla de tarifas que fija la agencia para el proyecto; cada clasificación de tu cuadrilla tiene que coincidir con ella. La verificamos antes de entregar el reporte, no después.',
		),
		array(
			'¿Qué pasa si un reporte llega tarde o incompleto?',
			'La agencia puede retener tu pago hasta que se corrija. Ese es justo el escenario que el ciclo certificado que llevamos busca evitar.',
		),
		array(
			'Mi obra es privada, no pública — ¿necesito esto?',
			'No. La nómina certificada aplica solo a contratos estatales o municipales; una obra privada necesita nómina estándar.',
		),
		array(
			'¿Qué necesitan de mí en cada ciclo?',
			'El número y la agencia del contrato, una vez, más las horas por trabajador y por clasificación en cada ciclo.',
		),
	),
...
$sdn_img_hero = sdn_attachment_image( content_url( '/uploads/2026/08/servicio-nomina-certificada.webp' ), 'large' );
...
<section class="sdn-surface sdn-surface--video text-paper border-b border-paper/15">
  ...
  <div class="sdn-layer mx-auto max-w-[1200px] px-3 pb-16 pt-16 lg:px-6 lg:pb-20 lg:pt-24">
    ...
    <p class="sdn-hero-fade font-mono text-[0.6875rem] uppercase tracking-[0.14em] text-rule">
      ...
      <span data-i18n="svc.certified-payroll.name" class="text-paper"><?php echo esc_html( $c['name'] ); ?></span>
    </p>

    <h1 data-i18n="svc.certified-payroll.h1" class="sdn-measure-sm mt-5 font-display text-[2rem] font-bold leading-[1.05] tracking-[-0.02em] text-paper sm:text-[2.75rem] lg:text-5xl">
    ...
    <p data-i18n="svc.certified-payroll.deck" class="sdn-hero-fade sdn-hero-fade--2 sdn-measure mt-8 text-[1.125rem] leading-[1.65] text-rule">
...
<div class="sdn-ticker" aria-hidden="true">
  <div class="sdn-ticker__track">
    <span class="sdn-ticker__item" data-i18n="svc.certified-payroll.name"><?php echo esc_html( $c['name'] ); ?></span>
        ...
        foreach ( $sdn_stages as $i => $stage ) :
          ...
          <article data-reveal="<?php echo esc_attr( $i * 60 ); ?>" data-sdn-step
            ...
            <div class="mt-4 min-w-0 lg:mt-0">
              <h3 data-i18n="svc.certified-payroll.st<?php echo esc_attr( $i + 1 ); ?>" class="font-display text-[1.375rem] font-semibold leading-tight text-ink sm:text-[1.625rem]">
                <?php echo esc_html( $stage[1] ); ?>
              </h3>

              <?php if ( $stage[2] ) : ?>
                <p data-i18n="svc.certified-payroll.<?php echo esc_attr( $sdn_stage_body_keys[ $i ] ); ?>" class="sdn-measure-sm mt-4 text-[1.0625rem] leading-[1.65] text-ink-2">
                ...
                <ul class="mt-5 space-y-3">
                  <?php foreach ( $stage[3] as $li => $line ) : ?>
                    <li class="sdn-measure-sm flex gap-4 text-[1.0625rem] leading-[1.65] text-ink-2">
                      ...
                      <span data-i18n="svc.certified-payroll.do.<?php echo esc_attr( $li ); ?>" class="min-w-0"><?php echo esc_html( $line ); ?></span>
                    </li>
                  <?php endforeach; ?>
                </ul>
              <?php endif; ?>
...
<!-- ══════════════ Glosario · solo en español ══════════════
     ...
     Antes era una <dl> de tres renglones planos — se pidió explícitamente
     más carácter. Ahora cada término es una ficha con acento de color
     (.sdn-panel-accent, misma pieza que "Cómo empieza" en home), un
     ícono propio y el término en una etiqueta tipo código — es
     literalmente jerga técnica, así que se lee como tal, no como
     prosa. Tres, no cuatro: no se infla la cuenta para llenar una
     cuadrícula pareja.
  ...
  <div class="sdn-layer mx-auto max-w-[1200px] px-3 py-14 lg:px-6 lg:py-16">
    <div data-reveal-group>
      ...
      <div class="sdn-reveal-stagger flex flex-wrap items-center gap-3">
        <p class="sdn-eyebrow font-mono text-[0.6875rem] uppercase tracking-[0.14em] text-muted">
          Glosario
        </p>
        <span class="sdn-tag" aria-hidden="true">3 términos</span>
      </div>
      ...
      $sdn_glossary = array(
      	array(
      		...
      		'def'  => 'El salario mínimo que el proyecto obliga a pagar por cada clasificación de trabajo. No es el salario mínimo del estado y no es el que tú acordaste: es el del contrato.',
      		'icon' => '<path d="M12 3v18"/><path d="M5 8h14"/><path d="m5 8-2.5 5A2.5 2.5 0 0 0 5 16a2.5 2.5 0 0 0 2.5-3L5 8Z"/><path d="m19 8-2.5 5a2.5 2.5 0 0 0 2.5 3 2.5 2.5 0 0 0 2.5-3L19 8Z"/>',
      	),
      	array(
      		'term' => 'Wage determination',
      		'def'  => 'El documento que fija esas tarifas para tu proyecto en concreto. Sale con el contrato y es contra lo que se coteja cada hora reportada.',
      		'icon' => '<path d="M14 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V8z"/><path d="M14 3v5h5"/><path d="M9 13h6"/><path d="M9 17h4"/>',
      	),
      	array(
      		'term' => 'WH-347',
      		'def'  => 'La forma federal de nómina certificada. Algunas agencias estatales y municipales aceptan su propio formato en lugar de esta; por eso lo primero que preguntamos es qué agencia otorgó el contrato.',
      		'icon' => '<rect x="4" y="4" width="16" height="16" rx="1.5"/><path d="M8 9h8"/><path d="M8 13h5"/>',
      	),
      );
      foreach ( $sdn_glossary as $gi => $g ) :
      	?>
        <div class="sdn-reveal-stagger sdn-panel-accent sdn-panel-accent--paper2 mt-4">
          <div class="flex items-start gap-4">
            <span class="sdn-ops-icon shrink-0" aria-hidden="true">
              <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><?php echo $g['icon']; ?></svg>
            </span>
            <div class="min-w-0">
              <span class="inline-flex rounded-sm border border-accent/35 bg-accent/10 px-2.5 py-1 font-mono text-[0.8125rem] font-semibold text-accent-2">
                <?php echo esc_html( $g['term'] ); ?>
              </span>
              <p class="sdn-measure mt-3 text-[0.9375rem] leading-relaxed text-ink-2">
                <?php echo esc_html( $g['def'] ); ?>
              </p>
            </div>
          </div>
        </div>
...
<section class="sdn-surface sdn-surface--paper border-b border-rule">
  ...
  <div class="sdn-layer grid lg:grid-cols-2 lg:items-stretch">
    ...
    <div class="px-3 py-16 lg:flex lg:flex-col lg:justify-center lg:px-9 lg:py-16 xl:px-12">
      <div data-reveal-group>
        ...
        <h2 data-i18n="svc.certified-payroll.faq_h2" class="sdn-reveal-stagger mt-4 font-display text-[1.75rem] font-semibold leading-[1.15] text-ink sm:text-[2.25rem]">
          <?php echo esc_html( $c['faq_h2'] ); ?>
        </h2>
        <p data-i18n="svc.certified-payroll.faq_note" class="sdn-reveal-stagger sdn-measure-sm mt-5 text-[0.9375rem] leading-relaxed text-muted">
        ...
        <?php foreach ( $c['faqs'] as $i => $faq ) : ?>
          <details class="sdn-faq2 sdn-reveal-stagger group" style="--faq-tone: <?php echo esc_attr( $sdn_tones[ $i % 3 ] ); ?>;">
            <summary class="flex cursor-pointer items-start gap-4 py-5">
              ...
              <span data-i18n="svc.certified-payroll.faq.<?php echo esc_attr( $i ); ?>.q" class="sdn-measure-sm flex-1 font-display text-[1.0625rem] font-semibold leading-snug text-ink transition-colors duration-150 group-hover:text-accent-2">
            ...
            <p data-i18n="svc.certified-payroll.faq.<?php echo esc_attr( $i ); ?>.a" class="sdn-measure-sm py-1 pb-6 pl-[2.25rem] text-[1.0625rem] leading-[1.65] text-ink-2">
  ...
  <div class="sdn-layer mx-auto max-w-[1200px] px-3 py-16 lg:px-6 lg:py-20">
    <div data-reveal-group class="lg:grid lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end lg:gap-16">
      ...
      <div class="sdn-reveal-stagger">
        <h2 data-i18n="svc.certified-payroll.end_h2" class="sdn-measure-sm font-display text-[1.75rem] font-semibold leading-[1.15] sm:text-[2.25rem]">
