<?php
/**
 * Template Name: Servicio — Tiempo y asistencia
 *
 * Soluciones del Norte · Tiempo y asistencia
...
$sdn_key = 'time-attendance';

$c = $is_en ? array(
	'crumb'   => 'Services',
	'name'    => 'Time and attendance',
	'h1'      => 'Hours get sorted before they reach payroll.',
	'deck'    => 'Hours and attendance tracking on a fixed format, so the payroll cycle doesn’t stall fixing data.',
	...
	'need'    => 'How you record hours today: paper, an app, text messages. We work from there, not from scratch.',
	'do'      => array(
		'Define a single capture format',
		'Review overtime and absences before close',
		'Flag inconsistencies for your review',
		'Deliver the file ready for the payroll run',
	),
	'get'     => 'The period’s hours report, already reconciled, before every payroll run.',
	'who'     => 'Businesses with field crews, rotating shifts or several job sites at once.',

	'more_l'  => 'The other six services',

	'faq_l'    => 'FAQ',
	'faq_h2'   => 'What people ask about time and attendance.',
	'faq_note' => 'If yours isn’t here, ask it on the intake call.',
	'faq_alt'  => 'Soluciones del Norte team member working on a laptop',
	'faqs'     => array(
		array(
			'What if my crew currently tracks hours on paper or by text?',
			'That’s fine — we work from whatever you use today and define a single capture format from there, not from scratch.',
		),
		array(
			'Does this replace payroll?',
			'No — it feeds it. The reconciled hours report is what goes into every payroll run, so the cycle doesn’t stall fixing data.',
		),
		array(
			'What do you do with overtime and absences?',
			'We review them before the close and flag inconsistencies for your review, so nothing surprising shows up in the payroll run.',
		),
		array(
			'Is this only for construction crews?',
			'It’s built for anyone coordinating hours across shifts or job sites — field crews are the most common case, not the only one.',
		),
	),
	...
	'h1'      => 'Las horas se ordenan antes de llegar a la nómina.',
	'deck'    => 'Registro de horas y asistencia con un formato fijo, para que el ciclo de nómina no se atore en corregir datos.',
	...
	'need'    => 'Cómo registras las horas hoy: papel, aplicación, mensaje de texto. Trabajamos desde ahí, no desde cero.',
	'do'      => array(
		'Definimos un formato único de captura',
		'Revisamos horas extra y ausencias antes del cierre',
		'Marcamos las inconsistencias para tu revisión',
		'Entregamos el archivo listo para el ciclo de nómina',
	),
	'get'     => 'El reporte de horas del periodo, ya conciliado, antes de cada ciclo de nómina.',
	'who'     => 'Negocios con cuadrillas en campo, turnos rotativos o varias obras al mismo tiempo.',

	'more_l'  => 'Los otros seis servicios',

	'faq_l'    => 'Preguntas',
	'faq_h2'   => 'Lo que se pregunta sobre tiempo y asistencia.',
	...
	'faqs'     => array(
		array(
			'¿Y si mi cuadrilla registra horas hoy en papel o por mensaje de texto?',
			'No hay problema — trabajamos desde lo que uses hoy y definimos un formato único de captura a partir de ahí, no desde cero.',
		),
		array(
			'¿Esto reemplaza la nómina?',
			'No — la alimenta. El reporte de horas ya conciliado es lo que entra a cada ciclo de nómina, para que el proceso no se atore corrigiendo datos.',
		),
		array(
			'¿Qué hacen con las horas extra y las ausencias?',
			'Las revisamos antes del cierre y marcamos las inconsistencias para tu revisión, para que no aparezca ninguna sorpresa en el ciclo de nómina.',
		),
		array(
			'¿Esto es solo para cuadrillas de construcción?',
			'Está pensado para cualquiera que coordine horas entre turnos u obras — las cuadrillas de campo son el caso más común, no el único.',
		),
	),
...
$sdn_img_hero = sdn_attachment_image( content_url( '/uploads/2026/08/servicio-tiempo-y-asistencia.webp' ), 'large' );
...
<section class="sdn-surface sdn-surface--video text-paper border-b border-paper/15">
  ...
  <div class="sdn-layer mx-auto max-w-[1200px] px-3 pb-16 pt-16 lg:px-6 lg:pb-20 lg:pt-24">
    ...
    <p class="sdn-hero-fade font-mono text-[0.6875rem] uppercase tracking-[0.14em] text-rule">
      ...
      <span data-i18n="svc.time-attendance.name" class="text-paper"><?php echo esc_html( $c['name'] ); ?></span>
    </p>

    <h1 data-i18n="svc.time-attendance.h1" class="sdn-measure-sm mt-5 font-display text-[2rem] font-bold leading-[1.05] tracking-[-0.02em] text-paper sm:text-[2.75rem] lg:text-5xl">
    ...
    <p data-i18n="svc.time-attendance.deck" class="sdn-hero-fade sdn-hero-fade--2 sdn-measure mt-8 text-[1.125rem] leading-[1.65] text-rule">
...
<div class="sdn-ticker" aria-hidden="true">
  <div class="sdn-ticker__track">
    <span class="sdn-ticker__item" data-i18n="svc.time-attendance.name"><?php echo esc_html( $c['name'] ); ?></span>
        ...
        foreach ( $sdn_stages as $i => $stage ) :
          ...
          <article data-reveal="<?php echo esc_attr( $i * 60 ); ?>" data-sdn-step
            ...
            <div class="mt-4 min-w-0 lg:mt-0">
              <h3 data-i18n="svc.time-attendance.st<?php echo esc_attr( $i + 1 ); ?>" class="font-display text-[1.375rem] font-semibold leading-tight text-ink sm:text-[1.625rem]">
                <?php echo esc_html( $stage[1] ); ?>
              </h3>

              <?php if ( $stage[2] ) : ?>
                <p data-i18n="svc.time-attendance.<?php echo esc_attr( $sdn_stage_body_keys[ $i ] ); ?>" class="sdn-measure-sm mt-4 text-[1.0625rem] leading-[1.65] text-ink-2">
                ...
                <ul class="mt-5 space-y-3">
                  <?php foreach ( $stage[3] as $li => $line ) : ?>
                    <li class="sdn-measure-sm flex gap-4 text-[1.0625rem] leading-[1.65] text-ink-2">
                      ...
                      <span data-i18n="svc.time-attendance.do.<?php echo esc_attr( $li ); ?>" class="min-w-0"><?php echo esc_html( $line ); ?></span>
                    </li>
                  <?php endforeach; ?>
                </ul>
              <?php endif; ?>
  ...
  <div class="sdn-layer grid lg:grid-cols-2 lg:items-stretch">
    ...
    <div class="px-3 py-16 lg:flex lg:flex-col lg:justify-center lg:px-9 lg:py-16 xl:px-12">
      <div data-reveal-group>
        ...
        <h2 data-i18n="svc.time-attendance.faq_h2" class="sdn-reveal-stagger mt-4 font-display text-[1.75rem] font-semibold leading-[1.15] text-ink sm:text-[2.25rem]">
          <?php echo esc_html( $c['faq_h2'] ); ?>
        </h2>
        <p data-i18n="svc.time-attendance.faq_note" class="sdn-reveal-stagger sdn-measure-sm mt-5 text-[0.9375rem] leading-relaxed text-muted">
        ...
        <?php foreach ( $c['faqs'] as $i => $faq ) : ?>
          <details class="sdn-faq2 sdn-reveal-stagger group" style="--faq-tone: <?php echo esc_attr( $sdn_tones[ $i % 3 ] ); ?>;">
            <summary class="flex cursor-pointer items-start gap-4 py-5">
              ...
              <span data-i18n="svc.time-attendance.faq.<?php echo esc_attr( $i ); ?>.q" class="sdn-measure-sm flex-1 font-display text-[1.0625rem] font-semibold leading-snug text-ink transition-colors duration-150 group-hover:text-accent-2">
            ...
            <p data-i18n="svc.time-attendance.faq.<?php echo esc_attr( $i ); ?>.a" class="sdn-measure-sm py-1 pb-6 pl-[2.25rem] text-[1.0625rem] leading-[1.65] text-ink-2">
  ...
  <div class="sdn-layer mx-auto max-w-[1200px] px-3 py-16 lg:px-6 lg:py-20">
    <div data-reveal-group class="lg:grid lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end lg:gap-16">
      ...
      <div class="sdn-reveal-stagger">
        <h2 data-i18n="svc.time-attendance.end_h2" class="sdn-measure-sm font-display text-[1.75rem] font-semibold leading-[1.15] sm:text-[2.25rem]">
