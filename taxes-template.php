...
$c = $is_en ? array(
	...
	'faq_l'    => 'FAQ',
	'faq_h2'   => 'What people ask about taxes.',
	...
	'faqs'     => array(
		array(
			...
			'As soon as your forms are ready — W-2, 1099, K-1 and whatever applies. We check what’s missing before we start, not the week of the deadline.',
		),
		array(
			'Will I know what I owe before you file?',
			'Yes — we tell you the outcome before filing, not after.',
		),
		array(
			'I have income from a state outside Oregon or Washington — can you still help?',
			'Say so on the intake call; it changes the scope, but we can typically still handle it.',
		),
		array(
			'Do you also do bookkeeping, or just the return?',
			'Both are separate services — if your books aren’t closed for the period yet, that’s the bookkeeping service, and it feeds directly into the return.',
		),
	),
	...
	'faq_l'    => 'Preguntas',
	'faq_h2'   => 'Lo que se pregunta sobre impuestos.',
	...
	'faqs'     => array(
		array(
			...
			'En cuanto tengas tus formas listas: W-2, 1099, K-1 y las que apliquen. Revisamos qué falta antes de empezar, no la semana de la fecha límite.',
		),
		array(
			'¿Voy a saber cuánto debo antes de que presenten?',
			'Sí — te decimos el resultado antes de presentar, no después.',
		),
		array(
			'Tengo ingresos de un estado fuera de Oregon o Washington — ¿pueden ayudarme igual?',
			'Dilo en la consulta inicial; cambia el alcance, pero por lo general sí podemos manejarlo.',
		),
		array(
			'¿También llevan contabilidad, o solo la declaración?',
			'Son servicios separados — si tus libros del periodo no están cerrados todavía, eso es contabilidad, y alimenta directo a la declaración.',
		),
	),
...
$sdn_img_hero = sdn_attachment_image( content_url( '/uploads/2026/08/servicio-impuestos.webp' ), 'large' );
...
<section class="sdn-surface sdn-surface--video text-paper border-b border-paper/15">
  ...
  <div class="sdn-layer mx-auto max-w-[1200px] px-3 pb-16 pt-16 lg:px-6 lg:pb-20 lg:pt-24">
    ...
    <p class="sdn-hero-fade font-mono text-[0.6875rem] uppercase tracking-[0.14em] text-rule">
      ...
      <span data-i18n="svc.taxes.name" class="text-paper"><?php echo esc_html( $c['name'] ); ?></span>
    </p>

    <h1 data-i18n="svc.taxes.h1" class="sdn-measure-sm mt-5 font-display text-[2rem] font-bold leading-[1.05] tracking-[-0.02em] text-paper sm:text-[2.75rem] lg:text-5xl">
    ...
    <p data-i18n="svc.taxes.deck" class="sdn-hero-fade sdn-hero-fade--2 sdn-measure mt-8 text-[1.125rem] leading-[1.65] text-rule">
...
<div class="sdn-ticker" aria-hidden="true">
  <div class="sdn-ticker__track">
    <span class="sdn-ticker__item" data-i18n="svc.taxes.name"><?php echo esc_html( $c['name'] ); ?></span>
        ...
        foreach ( $sdn_stages as $i => $stage ) :
          ...
          <article data-reveal="<?php echo esc_attr( $i * 60 ); ?>" data-sdn-step
            ...
            <div class="mt-4 min-w-0 lg:mt-0">
              <h3 data-i18n="svc.taxes.st<?php echo esc_attr( $i + 1 ); ?>" class="font-display text-[1.375rem] font-semibold leading-tight text-ink sm:text-[1.625rem]">
                <?php echo esc_html( $stage[1] ); ?>
              </h3>

              <?php if ( $stage[2] ) : ?>
                <p data-i18n="svc.taxes.<?php echo esc_attr( $sdn_stage_body_keys[ $i ] ); ?>" class="sdn-measure-sm mt-4 text-[1.0625rem] leading-[1.65] text-ink-2">
                ...
                <ul class="mt-5 space-y-3">
                  <?php foreach ( $stage[3] as $li => $line ) : ?>
                    <li class="sdn-measure-sm flex gap-4 text-[1.0625rem] leading-[1.65] text-ink-2">
                      ...
                      <span data-i18n="svc.taxes.do.<?php echo esc_attr( $li ); ?>" class="min-w-0"><?php echo esc_html( $line ); ?></span>
                    </li>
                  <?php endforeach; ?>
                </ul>
              <?php endif; ?>
  ...
  <div class="sdn-layer grid lg:grid-cols-2 lg:items-stretch">
    ...
    <div class="px-3 py-16 lg:flex lg:flex-col lg:justify-center lg:px-9 lg:py-16 xl:px-12">
      <div data-reveal-group>
        ...
        <h2 data-i18n="svc.taxes.faq_h2" class="sdn-reveal-stagger mt-4 font-display text-[1.75rem] font-semibold leading-[1.15] text-ink sm:text-[2.25rem]">
          <?php echo esc_html( $c['faq_h2'] ); ?>
        </h2>
        <p data-i18n="svc.taxes.faq_note" class="sdn-reveal-stagger sdn-measure-sm mt-5 text-[0.9375rem] leading-relaxed text-muted">
        ...
        <?php foreach ( $c['faqs'] as $i => $faq ) : ?>
          <details class="sdn-faq2 sdn-reveal-stagger group" style="--faq-tone: <?php echo esc_attr( $sdn_tones[ $i % 3 ] ); ?>;">
            <summary class="flex cursor-pointer items-start gap-4 py-5">
              ...
              <span data-i18n="svc.taxes.faq.<?php echo esc_attr( $i ); ?>.q" class="sdn-measure-sm flex-1 font-display text-[1.0625rem] font-semibold leading-snug text-ink transition-colors duration-150 group-hover:text-accent-2">
            ...
            <p data-i18n="svc.taxes.faq.<?php echo esc_attr( $i ); ?>.a" class="sdn-measure-sm py-1 pb-6 pl-[2.25rem] text-[1.0625rem] leading-[1.65] text-ink-2">
  ...
  <div class="sdn-layer mx-auto max-w-[1200px] px-3 py-16 lg:px-6 lg:py-20">
    <div data-reveal-group class="lg:grid lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end lg:gap-16">
      ...
      <div class="sdn-reveal-stagger">
        <h2 data-i18n="svc.taxes.end_h2" class="sdn-measure-sm font-display text-[1.75rem] font-semibold leading-[1.15] sm:text-[2.25rem]">
